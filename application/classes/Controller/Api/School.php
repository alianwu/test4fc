...
class Controller_Api_school extends Controller_Api {
  ...
  public function action_search()
  {
    ...
    $data = $this->model_school->get_list_front($this->city_id, $data);
    if ($data) {
  ...
  public function action_near()
  {
    ...
    if ($geo) {
      $data = $this->model_school->get_near_front($city_id, $lat, $lng, $radius, $page);
    }
    else {
      $where = array('page'=>$page);
      $data = $this->model_school->get_list_front($this->city_id, $where);
    }
    if ($data) {
      $this->result(0, $data->as_array());
    }
  ...
  public function action_hot()
  {
    ...
    $data = $this->model_school->get_hot_front($this->city_id, $page);
    if ($data) {
      $this->result(0, $data->as_array());
    }
    else {
  ...
  {
    $page = max((int) Arr::get($_GET, 'page', 1), 1);
    $data = $this->model_school->get_latest_front($this->city_id, $page);
    if ($data) {
      $this->result(0, $data->as_array());
    }
    else {
  ...
  public function action_favorite()
  {
    ...
    if ($ids) {
      $ids = explode('|', $ids);
      $data = $this->model_school->get_list_favorite($this->city_id, $ids);
      if ($data) {
        $this->result(0, $data->as_array());
      }
      else {
  ...
  public function action_article()
  {
    ...
    $category_id = $this->core->school_article_id;
    $where['page'] = $page;
