...
class Model_Config extends Model {
  ...
  {
    $query = DB::query(Database::SELECT, 'SELECT  name FROM config 
        WHERE name=:name AND "group"=:group LIMIT 1')
      ->param(':name', $name)
      ->param(':group', $group)
      ->execute();
    if ($query->count()) {
      $query = DB::query(Database::UPDATE, ' UPDATE config SET value=:value 
      ...
      $query = DB::query(Database::UPDATE, 'INSERT INTO config (name, value, "group") 
